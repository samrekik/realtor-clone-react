...
export default function SignIn() {
  ...
  return (
    <section>
      ...
      >
        {showPss ? <AiFillEyeInvisible size={24} /> : <AiFillEye size={24} />}
        </button>
            </div>
            <div className="flex justify-between" >
              <p >Don't have a account?<Link to="/sign-up"className="text-red-600 hover:text-red-700">Register</Link></p>
              <p><Link to="/forgot-password">Forgot Password?</Link></p>
            </div>
            <button
              type="submit"
              className="w-full bg-blue-500 text-white font-semibold py-2 my-5 rounded-md hover:bg-blue-600 transition duration-300"
            >
              Se connecter
            </button>
          </form>
